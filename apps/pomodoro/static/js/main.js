...
function updateDisplay() {
    ...
    currentPhase.textContent = isWork ? 'Work' : (currentCycle % totalCycles === 0 ? 'Long Break' : 'Break');
    ...
    if (!timerInterval) {
        timerInterval = setInterval(() => {
            ...
            if (remainingSeconds <= 0) {
                if (isWork) {
                    currentCycle++;
                    remainingSeconds = (currentCycle % totalCycles === 0 ? longBreakMinutes : breakMinutes) * 60;
                } else {
...
startBtn.addEventListener('click', (e) => {
    ...
    workMinutes = parseInt(document.getElementById('pomodoro_length').value) || 25;
    breakMinutes = parseInt(document.getElementById('break_time').value) || 5;
    longBreakMinutes = parseInt(document.getElementById('long_break').value) || 15;
